...
test.describe('Writer mode passkey unlock', () => {
  test('uses session read mode by default and unlocks writer mode only for matching passkey DID', async ({
    ...
    // Default behavior: read-only with temporary session identity.
    await expect(bobPage.getByTestId('post-form')).toHaveCount(0);
    await expect(
      bobPage.getByRole('button', { name: 'Authenticate With Passkey' })
    ).toHaveCount(0);
    ...
    await bobPage.getByRole('button', { name: 'Authenticate With Passkey' }).click();
