...
import { setupBlogSettings, createPost, getBlogAddress, waitForPeerConnection } from './test-utils';

test.describe('Bob Accessing Alice\'s Bach Blog', () => {
    let pageAlice;
    let pageBob;
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        await waitForPeerConnection(pageAlice);

        // Set up blog settings
        await setupBlogSettings(
            pageAlice,
            'The Complete Bach Chronicle',
            'A comprehensive exploration of J.S. Bach\'s life, works, and lasting influence',
            [
                'Sacred Works',
                'Keyboard Works',
                'Orchestral Works',
        ...
        for (const post of bachPosts) {
            await createPost(pageAlice, post);
        }

        // Get Alice's blog address
        aliceBlogAddress = await getBlogAddress(pageAlice);
        console.log("aliceBlogAddress", aliceBlogAddress);
        // Close Alice's page
        await pageAlice.close();
    });

    test('Bob can access and view Alice\'s Bach blog posts', async () => {
        ...
        await expect(pageBob.getByTestId('blog-name')).toHaveText('The Complete Bach Chronicle');
        //wait for 10 seconds
        await new Promise(resolve => setTimeout(resolve, 10000));
        await expect(async () => {
            const posts = await pageBob.getByTestId('post-item-title').all();
            console.log(posts.length);
            expect(posts.length).toBe(49);

            // Verify some specific post titles
            const postTitles = await Promise.all(posts.map(post => post.textContent()));
            expect(postTitles).toContain("St. Matthew Passion (BWV 244): The Greatest Story Ever Told");
            expect(postTitles).toContain("Mass in B Minor (BWV 232): Bach's Universal Statement of Faith");
            expect(postTitles).toContain("Brandenburg Concertos (BWV 1046-1051): The Perfect Six");
        ...
        for (const category of categories) {
            const categoryElements = await pageBob.getByText(category).all();
