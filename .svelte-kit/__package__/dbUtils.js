...
export async function addRemoteDBToStore(address, peerId, name) {
    ...
    if (!voyagerInstance)
        throw new Error("Voyager not initialized");
    try {
        let settingsDb;
        let newDB;
 ...
 */
export async function switchToRemoteDB(address, showModal = false) {
    let retry = true;
    ...
    try {
        while (retry && !cancelOperation) {
            ...
            if (blogNameValue)
                blogName.set(blogNameValue);
            if (blogDescriptionValue)
                blogDescription.set(blogDescriptionValue);
            if (postsDBAddressValue)
                postsDBAddress.set(postsDBAddressValue);
            if (categoriesValue)
                categories.set(categoriesValue); // Set categories
