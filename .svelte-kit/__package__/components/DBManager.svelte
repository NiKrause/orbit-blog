...
import { settingsDB, postsDB, remoteDBs, selectedDBAddress, orbitdb, remoteDBsDatabases, identity, identities } from "../store";
...
$: {
  if ($remoteDBs) {
    ...
    if (hasQueuedDBs && !queueCheckInterval) {
      ...
      queueCheckInterval = window.setInterval(processQueue, 30 * 1e3);
      processQueue();
...
async function scanQRCode() {
  ...
  try {
    ...
    if (ctx && videoElement.videoWidth > 0) {
      ...
      const scannedText = await new Promise((resolve) => {
        setTimeout(() => {
  ...
  try {
    ...
    for (const db of dbsToFetch) {
      try {
        ...
        db.lastProcessed = (/* @__PURE__ */ new Date()).toISOString();
        ...
        db.lastAttempt = (/* @__PURE__ */ new Date()).toISOString();
        await $remoteDBsDatabases.put({ _id: db.id, ...db });
      }
    }
    $remoteDBs = [...$remoteDBs];
...
async function handleSwitchToRemoteDB(address) {
  isModalOpen = true;
...
async function removeRemoteDB(id) {
